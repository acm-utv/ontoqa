...
public class LemonElementModel<Elem extends LemonElement> extends URIElement implements LemonElement{
	 ...
	 public static final URI RDF_TYPE = URI.create("http://www.w3.org/1999/02/22-rdf-syntax-ns#type");
	    ...
	    protected List<LemonElementModel<?>> referencers = new LinkedList<LemonElementModel<?>>();
	    private final HashMap<URI, Collection<Object>> annotations = new HashMap<URI, Collection<Object>>();
	    private final String modelName;
	    protected final LemonModelSimple model;
	    protected boolean checkRemote;

	protected LemonElementModel(String id, String type, LemonModelSimple model) {
		super(id);
    ...
    protected void setStrElemDirect(String name, LemonElement elem) {
        checkRemote = false;
        if (elem != null && elem instanceof LemonElementModel) {
            ((LemonElementModel<?>) elem).addReference(this);
        }
        if (elem != null) {
    ...
    protected void setStrElem(String name, LemonElement elem) {
        ...
        if (strElem.containsKey(name)) {
            if (strElem.get(name) instanceof LemonElementModel) {
                ((LemonElementModel<?>) strElem.get(name)).removeReference(this);
            }
        }
        if (model.allowUpdate()) {
            if (getURI() != null) {
    ...
    protected boolean addPredElemDirect(LemonPredicate p, LemonElement e) {
        checkRemote = false;
        if (e instanceof LemonElementModel) {
            ((LemonElementModel<?>) e).addReference(this);
        }
        if (!predElems.containsKey(p)) {
    ...
    protected boolean removePredElem(LemonPredicate p, LemonElement e) {
        ...
        if (e instanceof LemonElementModel) {
            ((LemonElementModel<?>) e).removeReference(this);
        }
        if (predElems.containsKey(p)) {
    ...
    ////////////////
    // Merge support
    protected void addReference(LemonElementModel<?> element) {
        referencers.add(element);
    }

    protected void removeReference(LemonElementModel<?> element) {
    ...
    protected void mergeIn(Elem elem) {
        ...
        for (Iterator<LemonElementModel<?>> it = lesp.referencers.iterator(); it.hasNext();) {
            LemonElementModel<?> rlesp = it.next();
    ...
    protected void printAsBlankNode(java.io.PrintWriter stream, SerializationState state) {
        ...
        for (LemonPredicate pred : predElems.keySet()) {
            ...
            for (LemonElement elem : predElems.get(pred)) {
                ...
                if (elem instanceof LemonElementModel) {
                    ((LemonElementModel) elem).visit(stream, state);
                    if (((LemonElementModel) elem).isMultiReferenced() && elem.getURI() != null) {
            ...
            for (LemonElement elem : elems) {
                if (elem instanceof LemonElementModel) {
                    if (!visitor.follow(URI.create(LemonModel.NEW_LEMON_URI + pred)) || follow(pred)) {
                        ((LemonElementModel) elem).accept(visitor);
                    }
                } else {
    ...
    public void clearAll() {
        for (Collection<LemonElement> elems : predElems.values()) {
            for (LemonElement lemonElement : elems) {
                if (lemonElement instanceof LemonElementModel) {
                    ((LemonElementModel) lemonElement).referencers.remove(this);
                }
            }
        }
    ...
    protected void defaultMerge(ReaderAccepter accepter, LinguisticOntology lingOnto, AccepterFactory factory) {
        ...
        if (accepter instanceof LemonElementModel) {
            final LemonElementModel<?> sle = (LemonElementModel<?>) accepter;
