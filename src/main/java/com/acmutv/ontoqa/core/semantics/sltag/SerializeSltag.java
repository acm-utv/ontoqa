...
public class SerializeSltag {
  ...
  private static List<String> auxiliaryVerb = Arrays.asList("do", "does", "did", "have", "has", "had");
	...
	{
		Ltag ltag = LtagTemplates.adjectiveSuperlative("most valuable", "the", "np");
	...
	{
		Ltag ltag = LtagTemplates.transitiveVerbPassiveIndicativeInterrogative(ppAdj,  "is","subj", "obj");
		Dudes dudes = DudesTemplates.property(predicateIRI, "subj", "obj");
	...
	public static ElementarySltag getSltagPreposition(String preposition,String predicateIRI){
		...
		Ltag ltag =  LtagTemplates.prepositionAdj(preposition, "subj", "obj");
		Dudes dudes = new SimpleDudes();
				...
				case commonNoun:
				{
					if(lEntry.getReferences().size() > 0 ) {
						  ...
						  if(ref!= null)
						  {
							  ...
							  grammar.addElementarySLTAG(SerializeSltag.getSltagRelPrepNoun(lEntry.getCanonicalForm(), "of", "obj", uri));
							  for(int j=0; j<lEntry.getForms().size(); j++)
							  {
								  ...
								  grammar.addElementarySLTAG(SerializeSltag.getSltagRelPrepNoun(lEntry.getForms().get(j).getWrittenRep(), "of", "obj", uri));
